...
export async function POST(request: Request) {
  ...
  if (!Array.isArray(scores) || scores.length === 0 || !scores.every((s) => typeof s === "number")) {
    return NextResponse.json({ error: "scores must be a non-empty array of numbers" }, { status: 400 });
  }

  const pointsEarned = computePronunciationPoints(scores);
  const now = new Date();
  ...
  try {
    ...
    console.error("POST /api/pronunciation failed", err);
    return NextResponse.json(
